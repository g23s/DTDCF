...
class LanguageEmbeddingLayer(nn.Module):
    ...
    def __init__(self, hp):
        ...
        bertconfig = BertConfig.from_pretrained('H:\why\MSATASE2\src\premodels/bert-base-uncased', output_hidden_states=True)
        # bertconfig = BertConfig.from_pretrained('/home/u2023111000/SIMSUF-main/src/premodels/bert-base-uncased', output_hidden_states=True)
        # self.bertmodel = BertModel.from_pretrained('/home/u2023111000/SIMSUF-main/src/premodels/bert-base-uncased', config=bertconfig)
        self.bertmodel = BertModel.from_pretrained('H:\why\MSATASE2\src\premodels/bert-base-uncased', config=bertconfig)
        # #
        # bertconfig = RobertaConfig.from_pretrained('/home/u2023111000/SIMSUF-main/src/premodels/roberta-base', output_hidden_states=True)
        # self.bertmodel = RobertaModel.from_pretrained('/home/u2023111000/SIMSUF-main/src/premodels/roberta-base', config=bertconfig)
...
class GRUEncoder(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, num_layers=1, dropout=0.2, bidirectional=False):
        ...
        self.gru = nn.GRU(in_size, hidden_size, num_layers=num_layers, dropout=dropout, bidirectional=bidirectional,
                           batch_first=False)
        self.dropout = nn.Dropout(dropout)
        self.linear_1 = nn.Linear((2 if bidirectional else 1) * hidden_size, out_size)
        self.linear_2 = nn.Linear((2 if bidirectional else 1) * hidden_size, out_size)
    ...
    def forward(self, x,y, lengths):
        ...
        packed_sequence = pack_padded_sequence(x, lengths, enforce_sorted=False)
        packed_sequencey = pack_padded_sequence(y, lengths, enforce_sorted=False)
        y0, final_states = self.gru(packed_sequencey,packed_sequence)
        ...
        if self.bidirectional:
            h1 = self.dropout(torch.cat((y0[0][0], y0[0][1]), dim=-1))
        else:
            h1 = self.dropout(y0[0].squeeze())
        y_2 = self.linear_1(h1)
        return y_1,y_2
